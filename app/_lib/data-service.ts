...
import { BookingType, GuestType } from "../_types/types";
...
export async function getBookings(guestId: string | null) {
  const { data, error, count } = await supabase
    .from("bookings")
...
export async function getBookedDatesByCabinId(cabinId: string | null) {
  let today = new Date();
...
export async function updateGuest(id: string, updateData: Partial<GuestType>) {
  const { data, error } = await supabase
    .from("guests")
    .update(updateData)
    .eq("id", id)
